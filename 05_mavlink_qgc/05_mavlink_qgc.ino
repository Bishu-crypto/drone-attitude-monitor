...
void calibrate(int samples = 1000) {
  Serial.println("Calibrating - keep FLAT and STILL...");
...
void sendMavlink(uint8_t* buf, uint16_t len) {
  if (qgcKnown) {
    udp.beginPacket(qgcIP, QGC_PORT);
    udp.write(buf, len);
    udp.endPacket();
  }
}
...
void setup() {
  ...
  udp.begin(QGC_PORT);
  Serial.println("UDP ready on port 14550");
  Serial.println("Waiting for QGC to connect...\n");
}
